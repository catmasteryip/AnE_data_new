...
ggboxplot(df, 
        ...
        ylab = "IRD", xlab = "Period")  + 
  scale_y_log10() + 
  stat_compare_means()

# density plot
ggplot(data = df, aes(x = deltaIR, col = period))+
  geom_density() +
  geom_vline(data = df, aes(xintercept = mean(deltaIR), col=period)) +
  xlim(-.05, .05) +
  # facet_wrap(~old_home, nrow=2) +
  # ylab("Cumulative portion") +
  xlab("Incidence rate reduction in treatment period, against control period") 
signi_f = function(x){
  signif(x, 3) 
}
df.covid %>%
