...
```{r}
results = compare_yearly(df) 

age_gender_attn = results[[2]] %>% 
  filter(ctrltrt == "ctrl", death==-1) %>% 
  select(-ctrltrt, -death) %>%
  arrange(year) 
write_xlsx(age_gender_attn, path = file.path("./results/age_gender_attn.xlsx"))
```
...
write_xlsx(age_gender_attn_wave, path = file.path("./results/age_gender_wave_attn.xlsx"))
```
...
rche_attn_wave = results %>% 
  ...
  select(-attn.y)
rche_attn_wave %>% View()
write_xlsx(rche_attn_wave, path = file.path("./results/rche_attn_wave.xlsx"))
```
...
results = compare_yearly(df = df, group.by = c("doadba", "demographic"), trtyrs = c(2021))
wave_elderly_df = results [[2]] %>% 
  filter(ctrltrt == "ctrl",
         death==1) %>%
  select(-ctrltrt, -death)
wave_elderly_df
write_xlsx(wave_elderly_df, path = "./results/wave_elderly_doadba_21.xlsx")
```
...
df = stata %>%
  filter(age >= 65) %>%
  select(old_home, year, eventdate, death, doadba) 
results = compare_yearly(df, group.by = c("doadba", "old_home"))
rche_df = results [[2]] %>% 
  filter(ctrltrt == "ctrl",
         death==1) %>%
  select(-ctrltrt, -death)
