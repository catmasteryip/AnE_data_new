...
# import dataset and subsetting from fig1.Rmd
...
selected =  stata %>% 
  select(c('eventdate',"DateofRegisteredDeath","age","Sex_num","death","old_home"))  %>% 
...
fig2b2 = ggplot() +
  geom_col(data = filter(cts.overall.age, death == 1, ctrltrt == "ctrl"), aes(x=cond,y=n, fill = ctrltrt), width = 0.5) +
  geom_col(data = filter(cts.overall.age, death == 1, ctrltrt == "trt"), aes(x=cond,y=n, fill = ctrltrt), width = 0.25) +
  geom_text(data = filter(cts.percentdiff.overall.age, death == 1, ctrltrt == "trt"),
            aes(x=cond, y=n, label=paste0(percent_diff,"%")),
            position=position_dodge(width=0.9),
            hjust = -.5,
            size=4) +
...
ggsave(plot=f2c,file=file.path(out_dir, "fig2c_raw.svg"),width=7,height=3.5)
...
write_xlsx(as.data.frame(ivreg_models_xlsx),path="./results/2sls_2016_2021.xlsx")
...
ggsave(plot=f2 & theme(text = element_text(size = 12.5)), file=file.path(out_dir,"fig2_raw.svg"), height = 9, width = 11.693)
```
